from sqlalchemy import Column, Integer, String, Boolean, DateTime, Text, Float, ForeignKey, JSON, Enum, UniqueConstraint
...
class CardMasterData(Base):
    ...
    edit_suggestions = relationship("EditSuggestion", back_populates="card_master", cascade="all, delete-orphan")
    card_documents = relationship("CardDocument", back_populates="card_master", cascade="all, delete-orphan")
...
class CardSpendingCategory(Base):
    ...
    __table_args__ = (UniqueConstraint("card_master_id", "category_name", name="uq_card_category"),)
...
class CardMerchantReward(Base):
    ...
    __table_args__ = (UniqueConstraint("card_master_id", "merchant_name", name="uq_card_merchant"),)
