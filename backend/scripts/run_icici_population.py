#!/usr/bin/env python3
"""
ICICI Bank Credit Cards Population Runner
Executes the SQL script to populate ICICI Bank credit cards data
"""
...
def verify_population(conn):
    """Verify that ICICI Bank cards were populated successfully"""
    try:
        cursor = conn.cursor()
        
        # Count ICICI Bank cards
        cursor.execute("SELECT COUNT(*) FROM card_master_data WHERE bank_name = 'ICICI Bank'")
        card_count = cursor.fetchone()[0]
        
        # Skip spending categories and merchant rewards for now
        category_count = 0
        merchant_count = 0
        
        # Get list of ICICI Bank cards
        cursor.execute("SELECT card_name FROM card_master_data WHERE bank_name = 'ICICI Bank' ORDER BY card_name")
        cards = cursor.fetchall()
        
        print("\n" + "="*60)
        print("ICICI BANK POPULATION VERIFICATION REPORT")
        print("="*60)
        ...
        print("\n📋 ICICI Bank Cards List:")
        for i, (card_name,) in enumerate(cards, 1):
            print(f"   {i:2d}. {card_name}")
        
        print("\n✅ ICICI Bank credit cards population completed successfully!")
...
def main():
    ...
    print("🏦 ICICI Bank Credit Cards Population Script")
    print("="*50)
    ...
    try:
        ...
        sql_script_path = os.path.join(script_dir, 'populate_icici_cards.sql')
        ...
        if execute_sql_script(conn, sql_script_path):
            ...
            print("❌ Failed to execute ICICI Bank population script")
            sys.exit(1)
